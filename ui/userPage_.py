...
#		self.axWidget.dynamicCall("Navigate(const QString&)",webstr)
	...
	def update(self):
		...
		self.historyIncompleteOrder = self.cursor.fetchall()
		if self.historyIncompleteOrder:
			orderID = self.historyIncompleteOrder[0][0]
			state = self.historyIncompleteOrder[0][1]
			if state == 0:
				self.label_deliverState.setText("Waiting for being accepted by restaurant")
			elif state == 1:
			...
			self.label_deliverState.setText("All Orders Delivered & Enjoy!")
...
#		if button == QMessageBox.Ok:
	...
	def courseClicked(self, index):
		if self.listWidget_course.currentRow() < 0:
			return
		courseName = self.listWidget_course.currentItem().text()
		if index >= 0:
	...
	def saveInfoChange(self):
		...
		if self.changeRecord == 1:
			try:
				num = self.tableWidget_Loc.rowCount()
				cmd = "insert into Loc (UserID, LocIdx, LocString, LocX, LocY) Values ({}, {}, '{}', {}, {});"\
					  .format(self.userID_, num+1, self.Address.toPlainText(), self.lineEdit_longitude.text(),\
...
# MYSQL
	...
	def insert_order(self, userID, restID):
		cmd1 = "insert into orders(UserID, RestID, LocIdx) values({}, {}, 1);".format(userID,restID)
